...
**Status:** **Merged into `2.10` on 9 September 2026**
**Merge commit:** [`276b7ab0`](https://github.com/thephpleague/commonmark/commit/276b7ab04d79062c5c36fad72f56314ff9f8ae0c)
...
**Status:** **Open / upstream review — rebased onto current `trunk` on 14 September 2026**
...
On 7 September 2026, the long-running branch had become non-mergeable against current `trunk`. The conflicting upstream change was isolated to WooCommerce's repository-wide replacement of legacy WPCS suppression comments in `class-wc-admin-webhooks-table-list.php`; the other three files touched by this PR had not changed on `trunk` since its merge base. The branch was reconciled by preserving the bulk-status implementation while adopting the current upstream PHPCS suppression form. On 11 September, upstream `trunk` had advanced again and GitHub marked the PR non-mergeable. The branch was rebased onto current `trunk` `e9ec926a017dcec3d4b2e0a61e90bb95d88e24a9`; Git dropped one now-redundant reconciliation commit because its patch was already upstream. The refreshed head `6c01b796c9a5633318d440445fd1405d62b49d73` is mergeable again. Later on 11 September, `trunk` advanced again to `adefb5f97154a7f0393c158cac56d27561028b31`; the branch was rebased again, producing head `5f93e9ec4c98c90cda52df2e6c3949f98aa57ef4`. GitHub reports the refreshed PR as mergeable, with milestone/test-reminder checks passing and CodeRabbit green. The final diff remains four files (135 additions, 3 deletions); `git diff --check` and PHP syntax checks for both changed PHP files pass. On 14 September, `trunk` advanced again to `24f946f0dc2c25eecd45983c2de9d3a0ca30d9c3`; the branch was rebased cleanly and force-pushed with an explicit lease, producing head `3411d4ded0c64ca8568919f67c2f3b7a83be747c`. GitHub again reports the PR as mergeable. `git diff --check` and PHP syntax checks for both changed PHP files pass after this rebase; the final diff remains four files (135 additions, 3 deletions).
...
**Status:** **Closed without merge on 14 September 2026**
**Related issue:** [#858](https://github.com/guzzle/psr7/issues/858)

The submission proposed extracting stream, request-mutation and URI-redaction operations from the broad `Utils` surface while preserving compatibility delegates. The branch had substantial local PHPUnit coverage and focused compatibility checks, but upstream process and scope alignment are authoritative for whether a contribution is useful.

A maintainer closed the PR without merge and explicitly asked that unsolicited AI-assisted PRs not be submitted to this repository, also stating that this proposal was incorrect. That feedback is now a hard project-specific gate: no further unsolicited contribution should be submitted to `guzzle/psr7` unless a maintainer explicitly invites the work and the intended scope is aligned before implementation.

**Why it matters:** technical validation alone does not make an upstream contribution appropriate. Maintainer consent, project direction and scope alignment are part of correctness, especially for API-surface changes.
